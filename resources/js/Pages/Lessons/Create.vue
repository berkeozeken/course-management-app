...
const submit = () => {
  form.post(`/sections/${props.section.id}/lessons`, { preserveScroll: true })
}
</script>

<template>
  <Head :title="`Yeni Ders - ${section.course?.title ?? ''}`" />
  <div class="max-w-3xl mx-auto p-6">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-xl font-semibold">Yeni Ders</h1>
      <Link :href="`/courses/${section.course_id}`" class="text-sm text-gray-600 hover:underline">Geri</Link>
    </div>
    ...
    <div class="rounded-2xl border p-4 space-y-4">
      ...
      <div>
        ...
        <div v-if="form.errors.video_url" class="text-sm text-red-600 mt-1">{{ form.errors.video_url }}</div>
      </div>
      ...
      <div class="flex gap-2">
        ...
        <Link :href="`/courses/${section.course_id}`" class="rounded-lg border px-4 py-2">Vazgeç</Link>
      </div>
    </div>
  </div>
</template>
